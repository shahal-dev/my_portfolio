  ...
  return (
    <header id="header" className="absolute top-0 z-50 w-full h-20">
      <div className="flex items-center justify-between h-full max-w-7xl pl-6 pr-4 mx-auto border-b border-l-0 border-r-0 border-transparent select-none lg:border-r lg:border-l lg:rounded-b-xl">
        <Link href="/" className="h-5 text-base group relative z-30 flex items-center space-x-1.5 font-semibold whitespace-nowrap text-black dark:text-white">
        ...
        <nav className={`relative z-30 flex flex-row-reverse justify-start w-full text-sm sm:justify-end sm:flex-row ${light ? 'text-neutral-300' : 'text-neutral-500 dark:text-neutral-400'}`}>
          <div
            id="openMenu"
            onClick={toggleMenu}
          ...
          >
            <svg
              className={`w-8 h-8 ${light ? 'text-neutral-200' : 'dark:text-neutral-200'}`}
              fill="none"
              strokeLinecap="round"
            ...
            {globalConfig.navigation.items.map((item) => (
              <Link
                ...
                className={`relative flex items-center justify-center w-full px-3 py-2 font-medium tracking-wide text-center duration-200 ease-out sm:py-0 sm:mb-0 md:w-auto ${
                  light ? 'hover:text-white' : 'hover:text-neutral-900 dark:hover:text-white'
                } ${
                  isActive(item.href) ? (light ? 'text-white' : 'text-neutral-900 dark:text-white') : ''
                }`}
              >
                {item.title}
              </Link>
